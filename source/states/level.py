...
class Level(tools.State):
    ...
    def load_map(self):
        ...
        path1 = str(__file__).split('\\')[0] + '\\' + str(__file__).split('\\')[1] + '\\' + str(__file__).split('\\')[
            2] + '\\source'
        file_path = os.path.join(path1, 'data', 'maps', map_file)
        f = open(file_path)
    ...
    def update_viewport(self):
        ...
        elif self.player.x_vel < 0 and player_center <= left_best and self.viewport.x > self.start_x:
            self.viewport.x += round(self.player.x_vel)
